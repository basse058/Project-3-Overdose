...
function barChartDeaths(dataD) {
    ...
    //   bary.append(data.nameIndex[dnI])
    // };//sample_values
    let barx = dataD['Number_of_Deaths_by_Year'];//otu_ids, use this to build string for chart
    // let barystr = []// use this for bar chart
    // bary.forEach(pear => barystr.push(`OTU_${pear}`));
    let barz = dataD['Number_of_Deaths_by_Year'];//otu_labels
    console.log(barx)
    console.log(bary)
    // console.log(barz)
    //color options
